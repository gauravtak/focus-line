...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function load() {
    chrome.storage.sync.get(['scroll_marker_settings'], (res) => {
      const s = Object.assign({}, defaultSettings, res.scroll_marker_settings || {});
  ...
  function save() {
    ...
    chrome.storage.sync.set({ scroll_marker_settings: s }, () => {
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0]) {
          chrome.tabs.sendMessage(tabs[0].id, { type: 'apply_settings', settings: s }, () => {});
        }
      });
    });
  }
  ...
  resetBtn.addEventListener('click', () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (!tabs[0]) return;
      const key = 'scroll_marker_page_' + new URL(tabs[0].url).origin + new URL(tabs[0].url).pathname;
      const toRemove = [key];
